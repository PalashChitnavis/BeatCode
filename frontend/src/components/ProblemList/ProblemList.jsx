...
const ProblemList = () => {
        ...
        const DifficultyTextStyles =(question)=> { return question.diff==='easy'? 'text-green-600':
                question.diff==='medium'? 'text-yellow-500':'text-red-600'
        } 
        ...
        const QuestionsArray = questions.map((question) => (
                <tr key={question.id} className="text-white font-bold text-center border-b-2 border-gray-400">
                        ...
                        <td className="w-[70%] p-4 text-left hover:text-indigo-900">
                                <Link to={`/practiceproblems/questions/${question.id}`}>{question.title}</Link>
                        </td>
                        <td className={`w-[20%] p-4 ${DifficultyTextStyles(question)}`}>{question.diff}</td> 
                </tr> 
        ))
